...
## agenda-20260520-004 MVP 운영 기준

이번 변경은 봇 판정 우회가 아니라 요청량 완화, 진단 로그 확보, 허용된 실행 환경에서의 안정화에 초점을 둡니다.

- 상세 페이지 기본 지연은 `--detail-delay-sec 3.0`이며 실제 대기에는 jitter가 적용됩니다.
- 상세 페이지 보강 범위는 `--max-detail-items`로 제한할 수 있습니다. `0`은 전체 상품 대상입니다.
- 카테고리 smoke test는 `--max-categories`로 범위를 제한합니다.
- 봇 확인 페이지가 감지되면 `output/logs/bot_detection_*.log`에 URL, 방식, 상태 코드, 감지 키워드, 본문 일부를 기록합니다.
- `--detail-pages auto`에서는 상세 페이지 봇 판정이 연속으로 발생하면 상세 보강을 중단하고 목록 기반 가격 보정만 적용합니다.
- GitHub Actions는 운영용 전체 수집이 아니라 smoke test 용도로 유지합니다. 전체 수집은 로컬 PC, 사내 서버, 공식/허용된 데이터 경로처럼 운영 정책을 확인할 수 있는 환경에서 실행합니다.
- 봇 확인 또는 차단 로그가 발생하면 같은 조건으로 반복 실행하지 말고, 지연 시간/수집 범위/실행 환경을 먼저 조정합니다.
- Circuit Breaker는 우회하지 않습니다. 연속 차단이 발생한 상태에서 강제로 상세 페이지 수집을 계속하면 IP 평판과 운영 안정성이 더 나빠질 수 있습니다.
- 수집 대상 사이트의 정책, 약관, 접근 허용 범위를 확인한 뒤 운영합니다.

예시:

```bash
python gmarket_best_crawler.py --max-categories 1 --max-items 5 --mode auto --skip-detail-pages
python gmarket_best_crawler.py --max-items 200 --mode auto --detail-pages auto --max-detail-items 20
```
